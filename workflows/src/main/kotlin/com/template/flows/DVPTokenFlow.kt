...
import com.r3.corda.lib.tokens.selection.database.config.PAGE_SIZE_DEFAULT as PAGE_SIZE_DEFAULT
...
@InitiatingFlow
@StartableByRPC
class DVPTokenFlow(private val tokenId: String,
...
@InitiatedBy(DVPTokenFlow::class)
class DVPAccountsHostedOnDifferentNodesResponder(val otherSide: FlowSession) : FlowLogic<SignedTransaction>() {
    ...
    override fun call():SignedTransaction {
        ...
        val queryCriteriaForSellerTicketType: QueryCriteria = QueryCriteria.VaultQueryCriteria(
        ...
        val queryCriteria: QueryCriteria = QueryCriteria.LinearStateQueryCriteria(uuid = listOf(UUID.fromString(ticketId)), status = Vault.StateStatus.UNCONSUMED)

        //grab the t20worldcup off the ledger
        val stateAndRef = serviceHub.vaultService.queryBy<CoinState>(criteria = queryCriteria).states[0]
        val evolvableTokenType: CoinState = stateAndRef.state.data
